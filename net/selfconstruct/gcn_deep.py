...
class PoseFeatureNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, hidden_channels=128, lstm_hidden_size=512, lstm_num_layers=1, gcn_layers=4):
        super(PoseFeatureNet, self).__init__()
        self.gcn = ResidualGCNBlock(in_channels, hidden_channels, num_layers=gcn_layers)
        self.fc_edge_features = nn.Linear(2 * 19, hidden_channels)  # Full connection layer for edge features
        self.bn = nn.BatchNorm1d(12)

        self.lstm = nn.LSTM(hidden_channels * 17 + 128, lstm_hidden_size, lstm_num_layers, batch_first=True,
                            bidirectional=True)
    ...
    def GCN(self, pose, hidden_channels=128):
        ...
        for time_step in range(seq_length):
            ...
            gcn_outputs.append(gcn_output.view(batch_size, num_nodes * hidden_channels))
