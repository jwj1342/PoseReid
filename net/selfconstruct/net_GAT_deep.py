...
class PoseFeatureNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, hidden_channels=128, lstm_hidden_size=512, lstm_num_layers=1,
        ...
        # 第一个GAT层
        self.gat1 = GATBlock(in_channels, hidden_channels, heads=gat_heads, dropout=dropout)
        # 新增的第二个GAT层
        self.gat2 = GATBlock(hidden_channels, hidden_channels, heads=gat_heads, dropout=dropout)
    ...
    def GAT(self, pose):
        ...
        for time_step in range(seq_length):
            ...
            gat_output = self.gat1(gat_input, self.connections)
            gat_output = self.gat2(gat_output, self.connections)  # 通过第二个GAT层
            gat_outputs.append(gat_output.view(batch_size, num_nodes * self.gat2.conv2.out_channels))
